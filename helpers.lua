...
function unfloat_and_move(c, s, t, ot)
    ...
    if ot ~= nil then
        for _, tag in pairs(ot) do c:toggle_tag(screen[s].tags[tag]) end
    end
end
...
function helpers.swapscreen(s)
    ...
    for _, c in pairs(screenClients) do c:move_to_screen(1) end
    for _, c in pairs(remoteClients) do c:move_to_screen(s) end
    awful.screen.focus(1)
end
